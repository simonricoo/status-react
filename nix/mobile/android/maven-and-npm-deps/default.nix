...
let
  mavenLocalRepo = callPackage ./maven { inherit localMavenRepoBuilder; stdenv = stdenv; };

  # Import the native dependencies for React Native Android builds
  react-native-deps = callPackage ./maven/reactnative-android-native-deps.nix { inherit stdenv; };
